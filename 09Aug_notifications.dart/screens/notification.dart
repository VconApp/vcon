...
class NotificationScreen extends StatefulWidget {
  ...
  const NotificationScreen({Key? key, this.initialTab}) : super(key: key);
...
class _NotificationScreenState extends State<NotificationScreen>
    with SingleTickerProviderStateMixin {
      ...
      void initState() {
        ...
        // Set the initial tab based on the initialTab parameter
        if (widget.initialTab != null) {
          _tabController.index = widget.initialTab == 'my_products' ? 0 : 1;
        }
      ...
      void handleNotificationTap(RemoteMessage message) {
        ...
        // Ensure we're not already on the NotificationScreen
        if (ModalRoute.of(context)?.settings.name != '/notification') {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (context) => NotificationScreen(
                initialTab: notificationType == 'my_products' ? 'my_products' : 'collect_products'
              ),
              settings: const RouteSettings(name: '/notification'),
            ),
          );
        } else {
          // If we're already on the NotificationScreen, just update the tab
          setState(() {
            _tabController.index = notificationType == 'my_products' ? 0 : 1;
          });
        }
      }
