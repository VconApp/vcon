...
class CollectProductScreenState extends State<CollectProductScreen> {
  ...
  Future<void> _refreshPage() async {
    ...
    await _fetchCollectRequest(collectorIRID);
    setState(() {
      _isLoading = false;
    });
  }
    ...
    if (confirmCollection == true) {
      ...
      try {
        ...
        if (snapshot.docs.isNotEmpty) {
          ...
          // Send OneSignal notification to the authorizer
          await OneSignalService.sendOneSignalNotification(
            _orders[collectionID]['authorizerIRID'],
            'Collection Help',
            '$collectorName is helping you to collect your product',
            'my_products',
          );

          // Send OneSignal notification to the collector (current user)
          await OneSignalService.sendOneSignalNotification(
            collectorIRID,
            'Collection Agreement',
            'You have agreed to help ${_orders[collectionID]['authorizerName']} collect their product',
            'collect_products',
          );

          print('Debug: OneSignal notifications sent for Help to Collect');
    ...
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        await OneSignalService.sendOneSignalNotification(
          _orders[collectionID]['authorizerIRID'],
          'Collection Rejected',
          'A collector has rejected to collect your product. Please choose another collector.',
          'my_products'
        );
        ...
        await OneSignalService.sendOneSignalNotification(
          collectorIRID,
          'Collection Rejected',
          'You have rejected to collect a product',
          'collect_products'
        );

        print('Debug: OneSignal notifications sent for Reject Collection');
