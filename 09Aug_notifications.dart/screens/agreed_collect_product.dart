...
class AgreedCollectProductsScreenState extends State<AgreedCollectProductsScreen> {
  ...
  Future<String> _getCollectorName(String collectorIRID) async {
    try {
      ...
      if (collectorDoc.exists) {
        return (collectorDoc.data() as Map<String, dynamic>)['irName'] ?? 'A collector';
      }
    } catch (e) {
  ...
  Future<void> _sendNotifications(String collectionID, String authorizerIRID, String authorizerName) async {
    try {
      String collectorName = await _getCollectorName(collectorIRID);

      // Notification to the authorizer
      await OneSignalService.sendOneSignalNotification(
        authorizerIRID,
        'Product Collected',
        '$collectorName has collected your product.',
        'my_products',
      );

      // Notification to the collector (current user)
      await OneSignalService.sendOneSignalNotification(
        collectorIRID,
        'Product Collected',
        'You have successfully collected the product for $authorizerName.',
        'collect_products',
      );
      ...
      await FirebaseFirestore.instance.collection('notifications').add({
        ...
        'body': '$collectorName has collected your product.',
      ...
      await FirebaseFirestore.instance.collection('notifications').add({
        ...
        'title': 'Product Collected',
        'body': 'You have successfully collected the product for $authorizerName.',
        'timestamp': FieldValue.serverTimestamp(),
        'isRead': false,
      });
      ...
      try {
        ...
        if (snapshot.docs.isNotEmpty) {
          await FirebaseFirestore.instance
            ...
            // Send notifications
            String authorizerIRID = _orders[collectionID]['authorizerIRID'];
            String authorizerName = _orders[collectionID]['authorizerName'];
            await _sendNotifications(collectionID, authorizerIRID, authorizerName);
        }
